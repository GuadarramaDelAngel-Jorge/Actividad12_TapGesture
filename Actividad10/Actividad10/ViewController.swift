    ...
    @IBAction func handleTap(_ gestureRecognizer:UITapGestureRecognizer){
            ...
            UIView.animate(withDuration: 0.50, animations: {
                if(self.pivot == 0){
                    ...
                    if let startingFrame = self.Image1.superview?.convert(self.Image1.frame, to: nil){
                    
                        let height = (self.view.frame.width / startingFrame.width) * startingFrame.height
                        let y = (self.view.frame.height / 2) - (height / 2)
                        self.Image1.frame = CGRect(x:0, y: y, width: self.view.frame.width, height: height)
                    }
